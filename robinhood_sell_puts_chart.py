...
for TICKER in safe_tickers:  # only safe tickers
    try:
        ...
        # Fetch historicals once and cache
        historicals = r.stocks.get_stock_historicals(TICKER, interval='day', span='month', bounds='regular')
        df = pd.DataFrame(historicals)
        if df.empty:
            send_telegram_message(f"📊 <b>{TICKER}</b> current: ${current_price:.2f}\n⚠️ No valid options found")
            continue
...
if all_options:
    ...
    df = historical_data_cache[best['Ticker']]
    last_14_low = df['low'][-LOW_DAYS:].min()
