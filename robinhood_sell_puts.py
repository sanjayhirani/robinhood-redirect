# robinhood_sell_puts.py - fully config-driven
...
def plot_candlestick(df, current_price, last_14_low, strikes=None, exp_date=None):
    ...
    if strikes:
        for s in strikes:
            ax.axhline(s, color=PLOT_CFG.get("strike_color","cyan"), linestyle='--', linewidth=1.5, label=f'Strike: ${s:.2f}')
    if exp_date:
...
def calculate_score(opt, buying_power):
    ...
    score = total_premium * iv_hv_ratio * liquidity_weight / (days**SCORING_CFG.get("days_exponent",1.0))
    return score
...
for ticker in TICKERS:
    try:
        ...
        msg_parts, has_event = [f"{TELEGRAM_LABELS.get('ticker_prefix','📊')} {ticker}"], False
        # Dividends
        try:
            ...
            if not future_divs.empty and future_divs.index.min().date() <= cutoff:
                msg_parts.append(f"{TELEGRAM_LABELS.get('dividend_alert','⚠️ 💰 Dividend on')} {future_divs.index.min().date().strftime('%d-%m-%y')}")
                has_event=True
        except: pass
        # Earnings
        try:
            ...
            if not ed.empty and today <= ed.index.min().date() <= cutoff:
                msg_parts.append(f"{TELEGRAM_LABELS.get('earnings_alert','⚠️ 📢 Earnings on')} {ed.index.min().date().strftime('%d-%m-%y')}")
                has_event=True
        except: pass
...
summary=[]
if risky_msgs: summary.append(f"{TELEGRAM_LABELS.get('risky_tickers','⚠️ Risky Tickers')}\n"+ "\n".join(risky_msgs))
...
for TICKER in safe_tickers:
    try:
        ...
        for exp_date in exp_dates:
            ...
            max_strikes = OPTION_FILTERS.get("max_selected_strikes",3)
            chosen_strikes = strikes_below[1:1+max_strikes] if len(strikes_below)>1 else strikes_below
            ...
            for opt in puts:
                ...
                if bid_price < OPTION_FILTERS.get("min_bid_price",0.10): continue
        ...
        selected_puts = sorted(candidate_puts, key=lambda x:x['COP Short'], reverse=True)[:max_strikes]
        all_options.extend(selected_puts)

        if selected_puts:
            msg_lines=[f"{TELEGRAM_LABELS.get('ticker_prefix','📊')} {TICKER} current: ${current_price:.2f}"]
            ...
            candidate_scores.append((best_put, calculate_score(best_put,buying_power)))
        else:
...
if candidate_scores:
    ...
    score_msg = f"<b>📊 Candidate Put Scores</b>\n💰 Available to invest: ${buying_power:,.2f}\n"
    for put, score in candidate_scores:
        ...
        score_msg += f"{put['Ticker']} | Exp: {put['Expiration Date']} | Strike: ${put['Strike Price']} | Max Contracts: {max_contracts} | Premium: ${total_premium:.2f} | Score: {score:.2f}\n"
    send_telegram(score_msg)
...
if all_options:
    ...
    msg_lines = [
        f"{TELEGRAM_LABELS.get('best_put','🔥 Best Cash-Secured Put')}: {best['Ticker']} current: ${best['Current Price']:.2f}",
        f"Exp: {best['Expiration Date']} | Strike: {best['Strike Price']} | Bid: ${best['Bid Price']:.2f} | Delta: {best['Delta']:.3f} | COP: {best['COP Short']*100:.1f}% | Max Contracts: {max_contracts} | Premium: ${total_premium:.2f}"
    ]
